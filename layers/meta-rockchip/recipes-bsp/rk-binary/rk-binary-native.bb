...
SRCREV_tools = "${@bb.utils.contains('TOOLS_RELEASE', 'NULL', \
		   '${FIREFLY_SDK_VERSION}', \
		   '${TOOLS_RELEASE}', \
		   d)}"

SRC_URI = " \
	git://gitlab.com/firefly-linux/rkbin.git;protocol=https;nobranch=1;branch=${FIREFLY_SDK_BRANCH};name=rkbin \
	git://gitlab.com/firefly-linux/tools.git;protocol=https;nobranch=1;branch=${FIREFLY_SDK_BRANCH};name=tools;destsuffix=git/extra \
"
